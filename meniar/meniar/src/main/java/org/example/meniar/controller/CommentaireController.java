...
import org.example.meniar.entities.Commentaire;
...
public class CommentaireController {
    ...
    private void onAddCommentClick() {
        ...
        Commentaire newComment = new Commentaire(0, content, new java.util.Date(), 1, forum.getIdForum());
        try {
    ...
    private void onUpdateCommentClick(Commentaire comment) {
        ...
        dialog.showAndWait().ifPresent(newContent -> {
            ...
            comment.setDatePublication(new java.util.Date()); // Optionally update date

            try {
                commentaireService.modifier(comment);
                loadComments(); // Reload comments to reflect the update
            } catch (SQLException e) {
