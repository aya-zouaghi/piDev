...
public class ReservationSalleService implements IService<Reservationsalle> {
    ...
    public List<Reservationsalle> getReservationsBySalleId(int salleId) throws SQLException {
        List<Reservationsalle> reservations = new ArrayList<>();
        String query = "SELECT * FROM reservation WHERE salle = ?";
        try (PreparedStatement pst = connection.prepareStatement(query)) {
    ...
    // Ajoutez cette méthode dans votre classe ReservationSalleService
    public Set<LocalDate> getReservedDatesForSalle(int salleId) throws SQLException {
        Set<LocalDate> reservedDates = new HashSet<>();
        String query = "SELECT date_debut FROM reservation WHERE salle = ?";
        try (PreparedStatement pst = connection.prepareStatement(query)) {
            pst.setInt(1, salleId);
            try (ResultSet rs = pst.executeQuery()) {
                while (rs.next()) {
                    reservedDates.add(rs.getDate("date_debut").toLocalDate());
                }
            }
        }
        return reservedDates;
    }
    public Reservationsalle getReservationForSalle(int salleId) throws SQLException {
        String query = "SELECT * FROM reservation WHERE salle = ? LIMIT 1";
        try (PreparedStatement pst = connection.prepareStatement(query)) {
            pst.setInt(1, salleId);
            try (ResultSet rs = pst.executeQuery()) {
                if (rs.next()) {
